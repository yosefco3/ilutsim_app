...
>
> עדכון אחרון: 13 יוני 2026 (📌 בורר השבוע בדיווחים ובייצוא נפתח כברירת-מחדל על השבוע הרלוונטי (open))
...
| תאריך     | שינוי                                                |
|-----------|-------------------------------------------------------|
| 13 יוני 2026 | 📌 **ברירת-מחדל לבורר השבוע גם בדף הייצוא = השבוע הרלוונטי** — אותה לוגיקה כמו בדיווחים הוחלה על `ExportPage`: השבוע ה-`open` נבחר אוטומטית בטעינה (`useRef` `didInitWeek`), כך שכפתור "הורד קובץ Excel" מאופשר מיד ללא בחירה ידנית; ללא שבוע פתוח — נשאר ב-prompt מושבת. כיסוי: +1 טסט ב-`exportPage.test.jsx` (ברירת-מחדל ל-open + כפתור מאופשר); frontend 129 (0 כשלים). |
| 13 יוני 2026 | 📌 **ברירת-מחדל לבורר השבוע בדיווחים = השבוע הרלוונטי** — דף `/submissions` נפתח כעת עם השבוע ה-`open` נבחר אוטומטית (במקום "בחר שבוע" ריק), כך שהאדמין רואה מיד את ההגשות הרלוונטיות וגם כפתור "מילוי אילוצים" מופיע ללא בחירה ידנית. `SubmissionsPage` משתמש ב-`useRef` (`didInitWeek`) כדי לבחור פעם אחת את `weeks.find(status==='open')` בטעינה; אם אין שבוע פתוח — נשאר ב-prompt, והאדמין עדיין יכול לבחור כל שבוע. frontend 128 (0 כשלים). |
| 13 יוני 2026 | 🎯 **כפתור "מילוי אילוצים" בדיווחים מופיע רק בשבוע הרלוונטי** — בעקבות ההעברה לדיווחים: הכפתור (ועמודת "פעולות") מוצג כעת **רק כשהשבוע הנבחר בבורר הוא בסטטוס `open`** — השבוע היחיד שניתן לעריכה. הדבר חוסם ויזואלית עריכה בשבוע **שפורסם** (`published`) ובשבוע **שכבר התחיל** (שעובר `auto-lock` ל-`locked` כש-`start_date ≤ today`), בהתאם ללוגיקת ה-backend הקיימת. `SubmissionsPage` מחשב `canFillConstraints = selectedWeek.status === 'open'` ומעביר ל-`StatusGrid`, ש מציג/מסתיר את העמודה (`colSpan` דינמי 4/5). ניקוי נלווה: `StatusGrid` עבר ל-`Fragment` עם `key` (הסרת אזהרת React). כיסוי: `statusGrid.test.jsx` (4 טסטים — מופיע ב-open, מוסתר אחרת, ברירת-מחדל מוסתר); frontend 128 (0 כשלים). |
| 13 יוני 2026 | 🔀 **העברת כפתור "מילוי אילוצים" ממסך מאבטחים למסך הדיווחים** — מיקום הגיוני יותר: הכפתור (עם כל הפונקציונליות וההגבלות) הוסר מ-`GuardTable` (ועמו prop `onFillConstraints` ו-`useNavigate` המיותר ב-`GuardsPage`) ונוסף כעמודת "פעולות" בכל שורה ב-`StatusGrid` (דף `/submissions`), המנווט ל-`/guards/:user_id/constraints` (זהה ל-`guardId` כי הפיילוד שומר `user_id: guardId`). יתרון נלווה: ניתן למלא גם עבור מאבטח שטרם הגיש בשבוע הנבחר. `AdminConstraintsPage` חוזר כעת ל-`/submissions` (כפתור "חזרה" + לאחר שמירה) במקום `/guards`. אין שינוי backend/API. כיסוי: frontend 124 (0 כשלים). |
| 13 יוני 2026 | 🛠️ **תיקון: דף לבן ריק למאבטח באנדרואיד — מעבר מהגשת Vite dev-server לבילד production** — `index.html` נטען (הכותרת הופיעה) אבל `#root` נשאר ריק → מודול ה-JS נכשל בטעינה/פרסור בדפדפן-הפנימי של טלגרם (אנדרואיד/MIUI). הסיבה: ה-frontend הוגש דרך **Vite dev server** (`dev.sh`+cloudflared tunnel) = גרף מודולי ESM **לא-מבונדל** (עשרות בקשות נפרדות + pre-bundling), ש-WebView ניידים נוטים להיכשל בו. `ErrorBoundary` לא תופס כשל ברמת טעינת מודול → מסך לבן בלי הודעה. **תיקון:** (1) **`dev.sh` מגיש כעת את הבילד** — `npm run build && npm run preview` במקום `npm run dev`, כך שהטאנל מגיש **באנדל יחיד מתורגם** (`dist/assets/index-*.js`, ~210KB) שה-WebView מפרסר אמין. (2) `vite.config.js`: בלוק **`preview`** חדש (port 3001 + `allowedHosts` + **`/api` proxy** — `vite preview` **לא** יורש את `server.*`, בלעדיו כל בקשה ל-API מחזירה 404) + `build.target: 'es2018'` לתאימות WebView ישן. `noCacheWebApp` כבר חל גם על preview (`index.html` → `no-cache`, אף פעם לא stale — הגנת ה-cache שתוקנה קודם נשמרת). (3) **מנגנון תפיסת מסך-לבן** ב-`index.html` (inline) — מצייר הודעת שגיאה קריאה ב-`#root` על כל `error`/`unhandledrejection` שלא-נתפס וגם כש-React לא עולה כלל (timeout 6ש'), כי `ErrorBoundary` לא מכסה כשלי טעינת/פרסור מודול. כיסוי: +1 טסט frontend (`indexHtml`: מנגנון תפיסת מסך-לבן); frontend 124, backend 211 (0 כשלים). **הערה:** האיבוד של HMR מקומי מקובל — מכונה זו מגישה production. |
| 13 יוני 2026 | 📲 **תיקון: WebApp נפתח כמסך כהה ריק למאבטח (כפתור `web_app`)** — `index.html` **לא טען את ה-SDK של Telegram WebApp** (`telegram.org/js/telegram-web-app.js`), כך ש-`Telegram.WebApp.ready()`/`expand()` מעולם לא נקראו. כשהמאבטח פתח דרך כפתור `web_app` ("ערוך אילוצים" ב-`submission_success_kb`/`SuccessPage`), טלגרם נשאר על מסך הטעינה הכהה שלו → "מסך כהה ריק". (פתיחה דרך כפתור `url` ב-`/start` עבדה כי זה דפדפן-פנימי רגיל, לא WebApp — מה שגם הסביר למה עריכות קודמות "עבדו".) **תיקון:** (1) הוספת תגית `<script src=".../telegram-web-app.js">` ב-`<head>` (בלוקינג, לפני מודול האפליקציה) — חושף `window.Telegram.WebApp` עם `initData` אמיתי (מתקן גם אימות ב-production, במקום ליפול ל-`__DEV_MODE__`). (2) `main.jsx` קורא `tg.ready()`+`tg.expand()` בעלייה (no-op מחוץ לטלגרם). (3) **`ErrorBoundary`** עוטף את האפליקציה — קריסת רינדור מציגה כעת מסך שגיאה קריא עם כפתור "רענן" במקום #root ריק (מסך כהה). כיסוי: +4 טסטי frontend (`errorBoundary`: ילדים/fallback; `indexHtml`: SDK קיים ולפני מודול האפליקציה); frontend 123 (0 כשלים). **הערה:** כפתור העריכה ב-`/start` (core.py) עדיין `url=` ולא `web_app=` — עובד בדפדפן-פנימי אך ללא initData; שדרוג ל-`web_app` הוא follow-up אפשרי. |
| 13 יוני 2026 | 🔢 **מונה הגשות אמיתי בדף השבועות (תיקון "0 הגשות" קבוע)** — `WeeksPage.jsx` קרא `w.submission_count`, אבל `WeekResponse` מעולם לא החזיר את השדה → תמיד הוצג **"0 הגשות"** גם כשהיו הגשות אמיתיות. נוסף `submission_count: int = 0` ל-`WeekResponse`; `SubmissionRepository.count_by_week()` מחזיר `{week_id: count}` בשאילתת `GROUP BY` **אחת** (ללא N+1), נחשף דרך `SubmissionService.get_submission_counts()`, ו-`GET /admin/weeks` ממלא את המונה לכל שבוע. כיסוי: `test_get_submission_counts_groups_by_week`; backend 211 (0 כשלים). |
| 13 יוני 2026 | ✅ **אישור/דחייה של הגשת מאבטח מגיעים מה-backend (לא "הצלחה" של הפרונט בלבד)** — בעקבות הבאג שבו מאבטח קיבל "נשלח בהצלחה" אף שההגשה לא הגיעה לשרת (קוד ישן מ-cache): כעת הצלחה מוכרזת **רק** כשה-backend מאשר שמירה אמיתית. **`useSubmission.submit`** מחזיר `{ok,error}` ומכריז `ok:true` רק כשתשובת `POST /submissions` כוללת הגשה ש**נשמרה עם `id`** — תשובת 2xx ללא גוף הגשה נחשבת כישלון (`ERR_NO_CONFIRM`), אף פעם לא "הצלחה" פרונטאלית. הניווט ל-`/submit/success` עבר ל-`SubmissionForm` (`handleSubmit`) ומותנה ב-`ok`. נוסף state **`submitting`** (כפתור מושבת + "שולח...") שמונע שליחה כפולה ותחושת "הצלחה מיידית". **`guardApiClient`** מציג כעת את **הודעת ה-`detail` של ה-backend** בכל דחייה (403/409/400/422) במקום מחרוזת פרונט גנרית (401 נשאר רמז-אימות ידידותי), ועבר לקבועי `messages.ERR_*`. כיסוי: +3 טסטי frontend (`useSubmission`: ok רק עם id, 2xx-ללא-גוף≠הצלחה; `guardApiClient`: detail מוצג, 401 ידידותי); frontend 119 (0 כשלים). |
| 13 יוני 2026 | 🧹 **תיקון: Telegram WebApp הגיש למאבטח גרסה ישנה (cached) → עריכה "נשלחה בהצלחה" אך לא הגיעה לשרת** — האבחון: השרת תקין לחלוטין (הוכח ע"י reproduction אמיתי מול ה-DB — עריכת מאבטח דורסת נכון מילוי אדמין לפי `(user_id, week_id)` ומופיעה בדיווחים), אבל ה-WebView של טלגרם שמר ב-cache גרסה ישנה של אזור המאבטח (`/submit`), שהוגש ב-production דרך **Vite dev/preview server** (`dev.sh` + cloudflared tunnel) עם נתיבי מודול **ללא hash** → cache "לנצח". **שני תיקונים:** (1) **כותרות no-cache** — plugin `noCacheWebApp` ב-`vite.config.js` (`configureServer`+`configurePreviewServer`) שמחזיר `Cache-Control: no-cache, no-store, must-revalidate` על כל מה שהשרת מגיש, כך שה-WebView תמיד מביא קוד עדכני. (2) **פרמטר גרסה ב-URL** — `app/version.py` (`APP_VERSION`=טוקן boot, משתנה בכל deploy) + helper מרכזי `app/bot/webapp.py::submit_webapp_url(tg_id=None)` המוסיף `?v={APP_VERSION}`; חובר לכל 3 מקומות ה-URL (`inline_kb.submission_success_kb`/`submit_constraints_kb`, `core.py` start handler), כך שטלגרם מתייחס לכתובת כחדשה אחרי כל deploy. כיסוי: `test_webapp_url.py` (4 טסטים) + עדכון `test_notification_on_open` (URL ממוגרס); backend 210, frontend 116 (0 כשלים). |
| 13 יוני 2026 | 🚫 **חסימת עריכת משמרות בשבוע שפורסם (אדמין כלול)** — למניעת התנגשויות אחרי פרסום, שבוע במצב `published` הפך **בלתי-ניתן לעריכה** גם דרך ממשק האדמין (`POST /submissions/admin`), שעד כה עקף כל סטטוס עם `override_lock=True`. **Backend:** `SubmissionService.create_submission` מזהה `WeekStatus.PUBLISHED` ומחזיר `WeekLockedException` עם הודעה ייעודית — **לפני** כל כתיבה, ללא תלות ב-`override_lock` (חוסם גם אדמין וגם מאבטח); עריכה ב-`locked`/`open`/`closed` נשארה מותרת. **Frontend:** `useAdminConstraints` חושף `isPublished`/`selectedWeek` ומחזיר `false` מ-`submit` לשבוע פורסם; `AdminConstraintsPage` משבית את שורות הימים (`DayRow disabled`), את ה-textarea ואת כפתור השליחה, ומציג באנר "השבוע פורסם — לא ניתן לערוך משמרות". נוסחת אזהרת ה"פרסם" עודכנה לנוסח המבוקש (שבוע חדש "נעול" יוצג + השבוע הנוכחי ייחסם לעריכה). כיסוי: 3 טסטי service חדשים (`test_admin_published_guard.py`: אדמין/מאבטח חסומים בפורסם, אדמין מותר בנעול) + טסט hook חדש (`isPublished` + `submit` no-op); backend 206, frontend 116 (0 כשלים). |
