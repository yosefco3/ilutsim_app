...
- **מתי**: בכל טעינה של דף השבועות (`GET /admin/weeks`)
- **מה קורה**:
  1. שבועות שהסתיימו (`end_date < today`) ולא פורסמו → מועברים אוטומטית ל-`PUBLISHED`
  2. אם אין שבוע פתוח לתקופה הנוכחית → נוצר שבוע חדש בסטטוס `OPEN`
- **מחיקת שבוע**: אפשרית רק לשבועות שלא פורסמו (שומר היסטוריה)
...
| 8 יוני 2026 | **תיקון קישור בהתראת פתיחת שבוע** — הוספת `/submit` ל-URL ב-`notify_week_opened()`, תיקון cloudflared tunnel מ-port 5173 ל-3001, הוספת `allowedHosts` ב-vite.config.js |
