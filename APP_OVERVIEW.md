...
>
> עדכון אחרון: 13 יוני 2026 (🛠️ תיקון דף לבן ריק במאבטח — מעבר מ-Vite dev-server לבילד production (`vite preview` מגיש את `dist/`) + מנגנון תפיסת מסך-לבן)
...
- **מילוי / עריכת אילוצים ע"י האדמין** — כפתור "מילוי אילוצים" בכל שורה פותח דף `/guards/:id/constraints` שבו האדמין בוחר שבוע, מסמן משמרות (בוקר/ערב/לילה) עם שעות לכל יום, ושומר עבור **כל** מאבטח (גם כזה שאין לו טלגרם). נשמר כ-`weekly_submission` רגיל; מותר בכל סטטוס שבוע **למעט פורסם** (`override_lock=True` — כולל סגור/פתוח/**נעול**, בורר השבוע מציג את הסטטוס). **שבוע שפורסם נחסם לעריכה** — גם לאדמין — למניעת התנגשויות אחרי פרסום הסידור: ה-service מחזיר `WeekLockedException`, ובפרונט הטופס מושבת עם באנר הסבר. **הגשה קיימת נטענת מראש לעריכה** — כולל הגשות שהמאבטח עצמו שלח דרך טלגרם — דרך `GET /submissions/admin?user_id=&week_id=`.
