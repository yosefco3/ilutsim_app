...
* **Identity = name, find-or-create.** Look up an existing guard by full name;
  if none, create one (default role ``BASIC_GUARD``, active). No rejection, no
  duplicate handling — uniqueness is guaranteed by policy.
...
class ConstraintsCommitService:
    ...
    async def _find_or_create_user(
        self, name: str, phone: str | None, name_map: dict[str, User]
    ) -> tuple[User, bool]:
    ...
    async def commit(
        ...
        *,
        ...
        for guard in parsed.guards:
            try:
                user, was_created = await self._find_or_create_user(
                    guard.name, guard.phone, name_map
                )
                if was_created:
                    created_new += 1
        ...
        preview = build_preview(parsed, existing_names)
