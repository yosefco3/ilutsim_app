...
already open or there is no closed candidate). Auto-lock: open → locked
silently, idempotent, never touches published weeks.
"""
...
async def test_auto_open_opens_upcoming_closed_week():
    ...
    repo.update.assert_awaited_once_with(closed.id, status=WeekStatus.OPEN)
...
@pytest.mark.asyncio
async def test_auto_lock_locks_open_week_silently():
    today = date.today()
